...
function tokenOf(u: { access_token?: string }): string {
  assert(
    ...
    'register returned no access_token',
  );
...
async function upload(
  ...
  const qs = filename === undefined
    ? ''
    : `?filename=${encodeURIComponent(filename)}`;
  return await matrix(
    new Request(`http://x/_matrix/media/v3/upload${qs}`, {
      method: 'POST',
      headers: {
...
Deno.test('M1/M2: upload → authed + legacy download — byte-equality, verbatim Content-Type', async () => {
  ...
  const dl = await matrix(
    new Request(`http://x/_matrix/client/v1/media/download/${SN}/${id}`, {
  ...
  const dl2 = await matrix(
    new Request(`http://x/_matrix/media/v3/download/${SN}/${id2}`),
  );
...
Deno.test('M1: no-token authed download → 401 before any lookup', async () => {
  const res = await matrix(
    new Request('http://x/_matrix/client/v1/media/download/hs1/garbage'),
  );
...
Deno.test('M4: async create → pending 504 → PUT 200 → second PUT 409 → downloads byte-equal', async () => {
  ...
  const tok = tokenOf(u);
  const cr = await matrix(
    new Request('http://x/_matrix/media/v1/create', {
      method: 'POST',
      headers: { Authorization: `Bearer ${tok}` },
    }),
  );
  assertEquals(cr.status, 200);
  const crBody = await cr.json() as {
    content_uri: string;
  ...
  const pending = await matrix(
    new Request(`http://x/_matrix/media/v3/download/${SN}/${id}`),
  );
  ...
  const put2 = await matrix(
    new Request(`http://x/_matrix/media/v3/upload/${SN}/${id}`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${tok}` },
    }),
  );
...
Deno.test('foreign server / unknown id → 404 M_NOT_FOUND', async () => {
  ...
  assertEquals(up.status, 200);
  const id = mediaIdOf((await up.json() as Json).content_uri as string);
  const foreign = await matrix(
    new Request(`http://x/_matrix/media/v3/download/evil.com/${id}`),
  );
  assertEquals(foreign.status, 404);
  assertEquals((await foreign.json() as Json).errcode, 'M_NOT_FOUND');
  const unknown = await matrix(
    new Request(`http://x/_matrix/media/v3/download/${SN}/doesnotexist00`),
  );
