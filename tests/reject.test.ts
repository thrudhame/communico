...
Deno.test('>2 prev_events: clean rejection (D1), no commit, branches intact', async () => {
  ...
  // E1, E2, E3 all with prev_events = [memberEventId] (the current
  // extremity); keepPrevBranches on the first two keeps that branch alive
  // for the next; E3 (normal) consumes it -> exactly three extremities:
  // x<E1>, x<E2>, x<E3>
  const ids: string[] = [];
