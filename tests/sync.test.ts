...
import { syncSince } from '../api/engine/syncfeed.ts';
...
Deno.test('syncSince: initial, incremental, and long-poll', async () => {
  ...
  // 2. initial sync: create + member + 2 messages, wire ids, seq order
  ...
  assertEquals(jr.timeline.events.length, 4);
  assertEquals(
    jr.timeline.events.map((e) => e.type),
    ['m.room.create', 'm.room.member', 'm.room.message', 'm.room.message'],
  );
  ...
  // state events present on initial sync (create + member)
  assertEquals(jr.state.events.length, 2);
  ...
  const r4 = await ingestEvent(ROOM, {
    type: 'm.room.message',
    sender: '@dev:localhost',
  ...
  await sleep(1000);
  const r5 = await ingestEvent(ROOM, {
    type: 'm.room.message',
    sender: '@dev:localhost',
