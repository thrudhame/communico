...
try {
  ...
  //    ≤15 s (proves the joiner-rescue: uncapped retry + onPeer trigger).
  {
    ...
    const pageC = await browser.newPage(`${BASE}/`);
    ...
    const pageD = await browser.newPage(`${BASE}/`);
  ...
  // 1. A creates, B joins
  const pageA = await browser.newPage(`${BASE}/`);
  ...
  const pageB = await browser.newPage(`${BASE}/`);
